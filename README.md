...
| `--mem-floor-gb` (env `DS4_MEM_FLOOR_GB`) | `4` | The engine never admits work that would leave the system under this many GiB of free memory: it reclaims idle cache first and refuses with a typed error if that is not enough. Lower it (down to 1) on a dedicated serving box to fund more context; raise it on a machine you also work on. The boot planning headroom now derives from this floor plus a measured boot-burst margin (boot line `batch fit headroom:`), so lowering the floor also returns planning margin to the fundable pool — `DS4_BATCH_FIT_HEADROOM_MB` pins the headroom outright. |
