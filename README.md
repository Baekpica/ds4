...
Four later increments pair the independent Gated DeltaNet recurrent updates
...
expert arithmetic remain independent. The same-process 3-by-3
...
24.20 tok/s (+1.74%), with all three paired rounds faster. The raw Q8 path
...
its exact target token streams. PLE projection/convolution, QSA, routed main
and shared MoE work, and the remaining GDN stages are still bank-owned.
...
| `DS4_QWEN_BATCH` | unset (off) | Set to `1` to enable Qwen's persistent two-bank lane. It supports exact-frontier and partial-prefix forks, bank disk-KV persistence, target-verified embedded MTP with `--mtp-draft 2`, image requests with decoded-pixel cache identity, bounded two-row token-embedding/Hyper-Connection/Q8-output decode, paired GDN recurrent and Q5_0 tail launches, and a combined SSD-PLE gather. PLE compute, QSA, routed main/shared MoE work, and the remaining GDN stages stay bank-owned. |
