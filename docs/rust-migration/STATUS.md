...
| KV store | yes | envelope/policy crate with payload-free metadata index, bounded LCP text reads, and a file-backed staged payload writer; ordinary serial Motif-3 no-think/no-tools visible cold + evict/replay checkpoints are production-integrated in `ds4-server-rs` | C | file-format 4-way + Rust streamed payload/trailer read by C + sparse 4 GiB indexing + embedded payload-range seam green; live same-model C/Rust 4-way save/load green at `cold=0, continued=0`; C-default 6,144-token cold partition has exact text/payload/output and C/Rust cross-read parity | same-policy restore TTFT ABBA green; scoped cold correctness/cross-read green but full default-policy perf pending; continued not ported |
...
ordinary serial replay above. The Rust server owns part of the HTTP host surface,
