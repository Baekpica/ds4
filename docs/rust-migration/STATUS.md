...
Rust proof evidence after the rendered-prompt fix is in `scratch/rust-host-live/proof-{smoke,long}-fixed.log` and `proof-oppc-{c-oracle,rust-fixed}.log`: smoke 2/2, long 6/6, and same-host OPP-C C/Rust 5/5 all pass. This does not bless the committed generic OPP-C golden: that file predates two scenario profiles and does not match the current C oracle. Remaining before Phase 9 includes a repeatable architecture-specific native drift gate, full user-binary parity, the family regression set, multi-client/server surface follow-ups, and pre-split performance/soak evidence. After Phase 9 + `SPLIT_READINESS.md`, follow [DFM_RS_SPLIT_PLAN.md](DFM_RS_SPLIT_PLAN.md).
