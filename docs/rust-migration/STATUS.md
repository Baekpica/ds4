...
| KV store | yes | isolated envelope/policy crate with payload-free metadata index and bounded LCP text reads; not production-integrated | C | file-format 4-way + sparse 4 GiB indexing + embedded payload-range seam green | n/a |
...
| 4 | KV store port + 4-way matrix | **format/policy + payload-free index + embedded range seam green** (`make test-kv-parity`, focused core/bridge oracles); production serial-server evict/save/restore and live same-model CUDA restore remain pending |
