...
| KV store | yes | envelope/policy crate with payload-free metadata index, bounded LCP text reads, and a file-backed staged payload writer; ordinary serial Motif-3 no-think/no-tools visible checkpoints are production-integrated in `ds4-server-rs` | C | file-format 4-way + Rust streamed payload/trailer read by C + sparse 4 GiB indexing + embedded payload-range seam green; live same-model C/Rust 4-way save/load green at `cold=0, continued=0` | restore TTFT pending; C-default cold/continued policy not ported |
