...
| KV store | yes | isolated envelope/policy crate with payload-free metadata index, bounded LCP text reads, and a file-backed staged payload writer; not production-integrated | C | file-format 4-way + Rust streamed payload/trailer read by C + sparse 4 GiB indexing + embedded payload-range seam green | n/a |
...
| 4 | KV store port + 4-way matrix | **format/policy + payload-free index + embedded range seam + replacement-safe staged writer green** (`make test-kv-parity`, focused core/bridge oracles); production serial-server evict/save/restore and live same-model CUDA restore remain pending |
