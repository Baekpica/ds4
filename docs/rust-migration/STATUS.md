...
| server (four surfaces) | yes | yes (`route_decide` + HTTP door + four parsers) | C | reason table + HTTP porcelain + parse C↔Rust green | n/a |
| distributed runtime | yes | yes (codecs + blocking orchestration) | C | codecs + CLI/route/mock hop green | n/a |
| CLI / bench / agent host | yes | shadow `ds4-rs` / `ds4-bench-rs` / `ds4-server-rs` | C | HTTP door + parsers green; generation still C | — |
...
| 7 | Server shadow by feature | **route_decide + HTTP door + four parsers green** (`make test-server-parity`); generation / continuation / `/metrics` `/v1/stats` still C |
...
| `ds4-server-rs` | Rust HTTP door + four JSON parsers (`make ds4-server-rs`); generation stays on C `ds4-server` |
