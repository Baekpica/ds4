...
int main(int argc, char **argv) {
    ...
    int oracle_n[2] = {0};
    for (int i = 0; i < 2; i++) {
        if (serial_greedy(engine, &prompt[i], 256, 2,
    ...
    for (int i = 0; !failed && i < 2; i++) {
        if (got[i].n_tokens != oracle_n[i] ||
            memcmp(got[i].tokens, oracle[i],
                   (size_t)oracle_n[i] * sizeof(int)) != 0) {
            fprintf(stderr,
    ...
    if (!failed && ds4_cont_bank_payload_bytes(ctx, 1u) != 0u) {
        fprintf(stderr, "EXAONE incorrectly advertised durable bank payloads\n");
        failed = 1;
    }
    ds4_batch_ctx_destroy(ctx);
