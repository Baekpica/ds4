...
int main(int argc, char **argv) {
    ...
    if (!weight_manifest || !weight_manifest[0]) {
        fprintf(stderr,
                "motif3-long: requires a live VMM weight owner; "
    ...
    /* Long gates run only as restartable workers over the persistent VMM
     * owner.  A stale shell variable must never revive the old full-copy path. */
