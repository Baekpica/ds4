/* Solar Open2 compressed GQA KV checks.
 *
 * Keep the production head width, exercise GQA head grouping, compare all
 * compressed formats with the BF16 oracle, and verify split decode against
 * the direct path over the same cache.
 */
#include "ds4_gpu.h"
...
static float cache_value(const uint8_t *row, ds4_solar_kv_format format,
    ...
    const uint16_t *scales = (const uint16_t *)(row + k_bytes + v_bytes);
...
int main(void) {
    CHECK(ds4_gpu_solar_kv_row_bytes(
              DS4_SOLAR_KV_BF16, T_HEAD_KV, T_DIM) == 1024u,
          "BF16 row layout");
    CHECK(ds4_gpu_solar_kv_row_bytes(
              DS4_SOLAR_KV_FP8, T_HEAD_KV, T_DIM) == 520u,
          "FP8 row layout");
    CHECK(ds4_gpu_solar_kv_row_bytes(
              DS4_SOLAR_KV_FP4, T_HEAD_KV, T_DIM) == 264u,
          "FP4 row layout");
    CHECK(ds4_gpu_solar_kv_row_bytes(
              DS4_SOLAR_KV_KFP8_VFP4, T_HEAD_KV, T_DIM) == 392u,
          "hybrid row layout");
    CHECK(ds4_gpu_solar_kv_row_bytes(
              DS4_SOLAR_KV_FP4, T_HEAD_KV, T_DIM - 1u) == 0u,
          "packed FP4 rejects odd head width");
    ...
    CHECK(ds4_gpu_solar_attention_prefill_tensor(
          ...
          "BF16 prefill");
    CHECK(ds4_gpu_tensor_read(out, 0, baseline_prefill,
    ...
    CHECK(ds4_gpu_solar_attention_decode_tensor(
          ...
          "BF16 decode");
    CHECK(ds4_gpu_tensor_read(decode, 0, baseline_decode,
                              row_count * sizeof(float)),
          "read BF16 decode");

    puts("== Solar Open2 compressed GQA KV ==");
    ...
    for (size_t fi = 0; fi < sizeof(formats) / sizeof(formats[0]); fi++) {
        ...
        CHECK(ds4_gpu_tensor_read(cache, 0, cache_host,
                                  (uint64_t)T_CAP * row_bytes),
        ...
        CHECK(ds4_gpu_solar_attention_decode_tensor(
              ...
              "compressed decode");
        CHECK(ds4_gpu_tensor_read(decode, 0, got_decode,
                                  row_count * sizeof(float)),
        ...
        CHECK(ds4_gpu_tensor_read(split, 0, got_split,
                                  row_count * sizeof(float)),
