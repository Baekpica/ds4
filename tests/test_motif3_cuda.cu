...
static void test_bf16_projection() {
    ...
    /* Force a resident device copy so this unit test exercises the same
     * weight-pointer resolver as a loaded GGUF without pinning vector pages. */
    setenv("DS4_CUDA_COPY_MODEL", "1", 1);
    if (!ds4_gpu_set_model_map(weight.data(), weight.size() * sizeof(uint16_t))) {
    ...
    if (!ds4_gpu_tensor_write(x.p, 0, input.data(), input.size() * sizeof(float)) ||
        !ds4_gpu_matmul_bf16_tensor(out.p, weight.data(),
