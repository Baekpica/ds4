...
typedef struct {
    ds4_gpu_tensor *q, *k, *v, *g, *beta, *out;
...
static void run_case(device_set *d, const float *q, const float *k,
    ...
    for (uint32_t part = 0; done < total; part++) {
        ...
        CHECK(ds4_gpu_solar_kda_prefill_tensor(
                  d->out, d->state, d->qc, d->kc, d->vc, d->q, d->k, d->v,
                  d->g, d->beta, d->qw, d->kw, d->vw, d->decay, d->dt,
                  n, T_HEAD, T_DIM, T_CONV, -5.0f),
              "prefill launch");
...
int main(void) {
    ...
    CHECK(d.q && d.k && d.v && d.g && d.beta && d.out && d.state && d.qc &&
