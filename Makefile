...
SOLAR_KDA_TEST := tests/test_solar_kda
...
DS4_SOLAR_KDA_FIXTURE ?= /workspace/solar-open2-mixed-ds4/fixtures/kda/synthetic-fla.bin
...
tests/test_solar_kda_fla: tests/test_solar_kda_fla.o ds4_cuda.o $(MMQ_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

test-solar-kda-fla: tests/test_solar_kda_fla
	./tests/test_solar_kda_fla "$(DS4_SOLAR_KDA_FIXTURE)"
...
tests/test_solar_session: tests/test_solar_session.o ds4_gpu_args.o ds4_kvstore.o rax.o $(CORE_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)
...
test-solar-session: tests/test_solar_session
	...
	DS4_SOLAR_KV_FORMAT=fp8 ./tests/test_solar_session "$(DS4_SOLAR_MODEL)"

tests/test_solar_lifecycle.o: tests/test_solar_lifecycle.c ds4.h
	$(CC) $(CFLAGS) -I. -I$(CUDA_HOME)/include -c -o $@ $<

tests/test_solar_lifecycle: tests/test_solar_lifecycle.o ds4_gpu_args.o ds4_kvstore.o rax.o $(CORE_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

test-solar-lifecycle: tests/test_solar_lifecycle
	@test -n "$(DS4_SOLAR_MODEL)" || \
		{ echo "set DS4_SOLAR_MODEL to a full Solar mixed GGUF" >&2; exit 2; }
...
tests/test_exaone_forward: tests/test_exaone_forward.o ds4_gpu_args.o ds4_kvstore.o rax.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_cuda.o ds4_layer_pack.o $(MMQ_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

tests/test_exaone_tokenizer.o: tests/test_exaone_tokenizer.c ds4.c ds4.h ds4_gpu.h
	$(CC) $(CFLAGS) -I. -I$(CUDA_HOME)/include -c -o $@ $<

tests/test_exaone_tokenizer: tests/test_exaone_tokenizer.o ds4_gpu_args.o ds4_kvstore.o rax.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_cuda.o ds4_layer_pack.o $(MMQ_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

tests/test_exaone_decode_batch.o: tests/test_exaone_decode_batch.c ds4.c ds4.h ds4_gpu.h
	$(CC) $(CFLAGS) -I. -I$(CUDA_HOME)/include -c -o $@ $<

tests/test_exaone_decode_batch: tests/test_exaone_decode_batch.o ds4_gpu_args.o ds4_kvstore.o rax.o ds4_distributed.o ds4_tp.o ds4_ssd.o ds4_cuda.o ds4_layer_pack.o $(MMQ_OBJS)
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)
endif

# Metadata-only mode accepts the converter's small vocab GGUF. The test
# includes ds4.c to exercise the real static loader, while section GC keeps the
# standalone diagnostic independent of the full runtime link graph.
...
test-solar-loader: tests/test_solar_loader
	@test -n "$(DS4_SOLAR_MODEL)" || \
		{ echo "set DS4_SOLAR_MODEL to a Solar vocab or full GGUF" >&2; exit 2; }
