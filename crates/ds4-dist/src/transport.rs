...
pub fn write_frame<W: Write>(w: &mut W, typ: u32, payload: &[u8]) -> io::Result<()> {
    let bytes = u32::try_from(payload.len()).map_err(|_| {
        io::Error::new(io::ErrorKind::InvalidInput, "distributed frame too large")
    })?;
...
pub fn read_frame<R: Read>(r: &mut R) -> io::Result<(u32, Vec<u8>)> {
    ...
    let h = decode_frame_header(&hdr).map_err(|e| match e {
        CodecError::BadMagic(m) => {
            io::Error::new(io::ErrorKind::InvalidData, format!("bad frame magic 0x{m:08x}"))
        }
