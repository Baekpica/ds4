...
use ds4_server::{
    accept_loop, accept_loop_with_engine, listen, model_id_from_gguf_path, NativeDecode,
    ServerConfig,
};
...
fn main() {
    ...
    if let Some(ref model) = model {
        ...
        accept_loop_with_engine(listener, cfg, &mut engine);
    } else {
...
fn usage() -> ! {
    eprintln!(
        "usage: ds4-server-rs --listen HOST PORT [--model-id ID] [-m GGUF] [--backend cuda|cpu|metal] [--tokens N] [-c N] [-t N] [--cors]"
    );
    std::process::exit(2);
}
